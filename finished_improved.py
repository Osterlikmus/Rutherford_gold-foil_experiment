...
def main():
    ...
    alfa_name = ["alfa_blue", "alfa_orange", "alfa_green", "alfa_red", "alfa_purple", "alfa_brown"]
    alfa_color = [BLUE, ORANGE, GREEN, RED, PURPLE, BROWN]
    alfa_startpos = [-4e-13, -2e-13, -1e-13, -0.5e-13, -0.2e-13, -0.1e-13]
    
    for index in range(len(alfa_name)):
            alfa_name[index] = Particle(-5e-13, alfa_startpos[index], 1, 6.64e-27, 3.20e-19, alfa_color[index])
            alfa_name[index].x_vel = 1.5e7
            particles = [gold, alfa_name[index]]

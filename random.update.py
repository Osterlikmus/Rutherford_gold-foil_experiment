...
# Helping function
def insideDisplay(alfa_particle):
    if(alfa_particle.x_pos < -alfa_particle.DIMENSION / 2 or alfa_particle.x_pos > alfa_particle.DIMENSION / 2 or alfa_particle.y_pos < -alfa_particle.DIMENSION / 2 or alfa_particle.y_pos > alfa_particle.DIMENSION / 2):
        return False
    else:
        return True
...
def main():
    ...
    gold = Particle(0, 0, 0, 5, 0, 1.26e-17, GOLD)
    # alfa_colors = [BLUE, ORANGE, GREEN, RED, PURPLE, BROWN]
    particles = [gold]

    while(running):
        alfa_startposition = random.uniform(-5e-13, 5e-13)
        # alfa_color = alfa_colors[random.randint(0, 6)]
        alfa_particle = Particle(-5e-13, alfa_startposition, 1.5e7, 1, 6.64e-27, 3.20e-19, WHITE)
        particles.append(alfa_particle)

        while(insideDisplay(alfa_particle)):
            # Updating of display
            DISPLAY.fill(BLACK)
            for particle in particles:
                particle.draw(DISPLAY)
            alfa_particle.update_position(gold)

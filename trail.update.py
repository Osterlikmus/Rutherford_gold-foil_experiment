...
def main():
    ...
    alfa_startpositions = [-4e-13, -2e-13, -1e-13, -0.5e-13, -0.2e-13, -0.1e-13]
    alfa_amount = len(alfa_colors)
    
    while(running):

        particles = [gold]

        for index in range(alfa_amount):
            alfa_particle = Particle(-5e-13, alfa_startpositions[index], 1.5e7, 1, 6.64e-27, 3.20e-19, alfa_colors[index])
